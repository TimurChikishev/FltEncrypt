...
PFLT_PORT CLIENT_PORT = NULL;
PFLT_PORT SERVER_PORT = NULL;
...
#define ENCRYPT_TAG      'encr'
#define DECRYPT_TAG      'decr'
#define RESULT_REPLACE_WORD_TAG 'rrwt'
#define SETTING_JSON_TAG 'sjso'
...
#pragma prefast(suppress:__WARNING_MEMORY_LEAK, "ctx->Name.Buffer will not be leaked because it is freed in CleanupVolumeContext")
        ...
        LOG_PRINT(LOGFL_VOLCTX,
            ("FltEncrypt!InstanceSetup:                  Real SectSize=0x%04x, Used SectSize=0x%04x, Name=\"%wZ\"\n",
                volProp->SectorSize,
                ctx->SectorSize,
                &ctx->Name));
...
CommunicateDisconnect(
    ...
    FltCloseClientPort(gFilterHandle, &CLIENT_PORT);

    CLIENT_PORT = NULL;
} 

NTSTATUS
FilterNotifyMessage(PVOID portCookie, PVOID inputBuffer, ULONG inputBufferLength, PVOID outputBuffer, ULONG outputBufferLength, PULONG retLength)
{  
    ...
    PCHAR status_success = "SUCCESS";
    PCHAR status_error = "ERROR";

    strcpy((PCHAR)outputBuffer, status_success);
    ...
    if ((inputBuffer == NULL) || (inputBufferLength > sizeof(INPUT_BUFFER)))
    { 
        return STATUS_INVALID_PARAMETER;
    }
     
    RtlCopyMemory(&inputBuf, inputBuffer, inputBufferLength);
    ...
    case ADD:  
        DbgPrint("ADD: \r\n");
        DbgPrint("FILE EXT: %ws \r\n", inputBuf.fileExt);
        DbgPrint("FILE NAME: %ws \r\n", inputBuf.fileName);
        DbgPrint("FILE PATH: %ws \r\n", inputBuf.filePath);
        DbgPrint("FILE PASSWORD: %ws \r\n", inputBuf.password);  
        break; 
    case DEL:  
        DbgPrint("DEL: \r\n");
        DbgPrint("FILE EXT: %ws \r\n", inputBuf.fileExt);
        ...
        break; 
    case CONNECT:
        DbgPrint("COMMAND: CONNECT \r\n");  
        break; 
    case DISCONNECT:
        DbgPrint("COMMAND: DISCONNECT \r\n");
        FltCloseClientPort(gFilterHandle, &CLIENT_PORT); 
        break; 
    default: 
        strcpy((PCHAR)outputBuffer, status_error); 
        break;
    } 
...
)
{
    ...
    UNICODE_STRING communicateName = { 0x00 };
    ...
    if (NT_SUCCESS(status)) {
        ...
        RtlInitUnicodeString(&communicateName, L"\\FLT_ENCRYPT_PORT");
    ...
    try {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreReadBuffers:             Error getting volume context, status=%x\n",
                    status));
        ...
        if (newBuf == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreReadBuffers:             %wZ Failed to allocate %d bytes of memory\n",
                    &volCtx->Name,
                    readLen));
        ...
        if (FlagOn(Data->Flags, FLTFL_CALLBACK_DATA_IRP_OPERATION)) {
            ...
            if (newMdl == NULL) {
                ...
                LOG_PRINT(LOGFL_ERRORS,
                    ("FltEncrypt!SwapPreReadBuffers:             %wZ Failed to allocate MDL\n",
                        &volCtx->Name));
        ...
        if (p2pCtx == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreReadBuffers:             %wZ Failed to allocate pre2Post context structure\n",
                    &volCtx->Name));
        ...
        LOG_PRINT(LOGFL_READ,
            ("FltEncrypt!SwapPreReadBuffers:             %wZ newB=%p newMdl=%p oldB=%p oldMdl=%p len=%d\n",
                &volCtx->Name,
                newBuf,
                newMdl,
...
int calcRoundBufferSize(int messageLen) {
    return (((messageLen / 1024) + 1) * 1024);
}
...
--*/
{
    ...
    try {
        ...
        if (!NT_SUCCESS(Data->IoStatus.Status) ||
            ...
            LOG_PRINT(LOGFL_READ,
                ("FltEncrypt!SwapPostReadBuffers:            %wZ newB=%p No data read, status=%x, info=%Iu\n",
                    &p2pCtx->VolCtx->Name,
        ...
        if (iopb->Parameters.Read.MdlAddress != NULL) {
            ...
            if (origBuf == NULL) {
                ...
                LOG_PRINT(LOGFL_ERRORS,
                    ("FltEncrypt!SwapPostReadBuffers:            %wZ Failed to get system address for MDL: %p\n",
                        &p2pCtx->VolCtx->Name,
                        iopb->Parameters.Read.MdlAddress));
            ...
            if (FltDoCompletionProcessingWhenSafe(Data,
                ...
                LOG_PRINT(LOGFL_ERRORS,
                    ("FltEncrypt!SwapPostReadBuffers:            %wZ Unable to post to a safe IRQL\n",
                        &p2pCtx->VolCtx->Name));
        ...
        try {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPostReadBuffers:            %wZ Invalid user buffer, oldB=%p, status=%x\n",
                    &p2pCtx->VolCtx->Name,
                    origBuf,
                    Data->IoStatus.Status));
        }
        ...
        if (NT_SUCCESS(status))
        {
                ...
                FALSE))
            {
                if (iopb->MajorFunction == IRP_MJ_READ) {
                    DbgPrint("[FltEncrypt] --------- read ---------");
                    DbgPrint("[FltEncrypt] size: %x\n", strlen((char*)origBuf));
                    DbgPrint("[FltEncrypt] Read Buffer: %s\n", (char*)origBuf);


                    if (strlen((char*)origBuf) != 0)
                    {
                        struct AES_ctx ctx;
                        ...
                        int bufferSize = calcRoundBufferSize(strlen((char*)origBuf));
                        char* hexarray = ExAllocatePoolWithTag(PagedPool, sizeof(char) * bufferSize, DECRYPT_TAG);

                        DbgPrint("[FltEncrypt] bufferSize: %d\n", bufferSize);

                        strncpy(hexarray, (char*)origBuf, bufferSize);


                        DbgPrint("[FltEncrypt] AES Encrypt: %s \n", (char*)hexarray);
                        AES_init_ctx_iv(&ctx, key, iv);
                        AES_CBC_decrypt_buffer(&ctx, (uint8_t*)hexarray, bufferSize);

                        DbgPrint("[FltEncrypt] AES Decrypt: %s\n", (char*)hexarray); 
                        strncpy((char*)origBuf, hexarray, bufferSize);
                

                        ExFreePoolWithTag(hexarray, DECRYPT_TAG);
                    }
                }
            }
        }
        ...
        //        if (iopb->MajorFunction == IRP_MJ_READ) {
        //            DbgPrint("[FltEncrypt] --------- read ---------");
        //            DbgPrint("[FltEncrypt] size: %x\n", strlen((char*)iopb->Parameters.Read.ReadBuffer));
        //            DbgPrint("[FltEncrypt] Read Buffer: %s\n", (char*)iopb->Parameters.Read.ReadBuffer);
        //            DbgPrint("[FltEncrypt] Write Buffer: %s\n", (char*)iopb->Parameters.Read.ReadBuffer);


        //            if (strlen((char*)iopb->Parameters.Read.ReadBuffer) != 0)
        //            {
        //                struct AES_ctx ctx;
        //                unsigned char k[] = "1234567890ABCDEF";    //16 bits
        //                uint8_t* key = (uint8_t*)k;
        ...
        //                char* readBuffer = (char*)iopb->Parameters.Read.ReadBuffer;
        //                int bufferSize = calcRoundBufferSize(strlen((char*)readBuffer));
        //                char* hexarray = ExAllocatePoolWithTag(PagedPool, sizeof(char) * bufferSize, DECRYPT_TAG);

        //                DbgPrint("[FltEncrypt] bufferSize: %d\n", bufferSize);

        //                strncpy(hexarray, readBuffer, bufferSize);
        ...
        //                strncpy((char*)iopb->Parameters.Read.ReadBuffer, hexarray, bufferSize);
        //                DbgPrint("[FltEncrypt] AES Decrypt: %s\n", (char*)iopb->Parameters.Read.ReadBuffer);
        ...
        if (cleanupAllocatedBuffer) {
            ...
            LOG_PRINT(LOGFL_READ,
                ("FltEncrypt!SwapPostReadBuffers:            %wZ newB=%p info=%Iu Freeing\n",
                    &p2pCtx->VolCtx->Name,
                    p2pCtx->SwappedBuffer,
                    Data->IoStatus.Information));
...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        LOG_PRINT(LOGFL_ERRORS,
            ("FltEncrypt!SwapPostReadBuffersWhenSafe:    %wZ Could not lock user buffer, oldB=%p, status=%x\n",
                &p2pCtx->VolCtx->Name,
                iopb->Parameters.Read.ReadBuffer,
                status));
        ...
        if (origBuf == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPostReadBuffersWhenSafe:    %wZ Failed to get system address for MDL: %p\n",
                    &p2pCtx->VolCtx->Name,
                    iopb->Parameters.Read.MdlAddress));
    ...
    LOG_PRINT(LOGFL_READ,
        ("FltEncrypt!SwapPostReadBuffersWhenSafe:    %wZ newB=%p info=%Iu Freeing\n",
            &p2pCtx->VolCtx->Name,
            p2pCtx->SwappedBuffer,
            Data->IoStatus.Information));
...
--*/
{
    ...
    try {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreDirCtrlBuffers:          Error getting volume context, status=%x\n",
                    status));
        ...
        if (newBuf == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreDirCtrlBuffers:          %wZ Failed to allocate %d bytes of memory.\n",
                    &volCtx->Name,
                    iopb->Parameters.DirectoryControl.QueryDirectory.Length));
        ...
        if (newMdl == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreDirCtrlBuffers:          %wZ Failed to allocate MDL.\n",
                    &volCtx->Name));
        ...
        if (p2pCtx == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreDirCtrlBuffers:          %wZ Failed to allocate pre2Post context structure\n",
                    &volCtx->Name));
        ...
        LOG_PRINT(LOGFL_DIRCTRL,
            ("FltEncrypt!SwapPreDirCtrlBuffers:          %wZ newB=%p newMdl=%p oldB=%p oldMdl=%p len=%d\n",
                &volCtx->Name,
                newBuf,
                newMdl,
...
--*/
{
    ...
    try {
        ...
        if (!NT_SUCCESS(Data->IoStatus.Status) ||
            ...
            LOG_PRINT(LOGFL_DIRCTRL,
                ("FltEncrypt!SwapPostDirCtrlBuffers:         %wZ newB=%p No data read, status=%x, info=%Ix\n",
                    &p2pCtx->VolCtx->Name,
                    p2pCtx->SwappedBuffer,
                    Data->IoStatus.Status,
                    Data->IoStatus.Information));
        ...
        if (iopb->Parameters.DirectoryControl.QueryDirectory.MdlAddress != NULL) {
            ...
            if (origBuf == NULL) {
                ...
                LOG_PRINT(LOGFL_ERRORS,
                    ("FltEncrypt!SwapPostDirCtrlBuffers:         %wZ Failed to get system address for MDL: %p\n",
                        &p2pCtx->VolCtx->Name,
                        iopb->Parameters.DirectoryControl.QueryDirectory.MdlAddress));
            ...
            if (FltDoCompletionProcessingWhenSafe(Data,
                ...
                LOG_PRINT(LOGFL_ERRORS,
                    ("FltEncrypt!SwapPostDirCtrlBuffers:         %wZ Unable to post to a safe IRQL\n",
                        &p2pCtx->VolCtx->Name));
        ...
        try {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPostDirCtrlBuffers:         %wZ Invalid user buffer, oldB=%p, status=%x, info=%Iu\n",
                    &p2pCtx->VolCtx->Name,
                    origBuf,
                    Data->IoStatus.Status,
                    Data->IoStatus.Information));
        ...
        if (cleanupAllocatedBuffer) {
            ...
            LOG_PRINT(LOGFL_DIRCTRL,
                ("FltEncrypt!SwapPostDirCtrlBuffers:         %wZ newB=%p info=%Iu Freeing\n",
                    &p2pCtx->VolCtx->Name,
                    p2pCtx->SwappedBuffer,
                    Data->IoStatus.Information));
...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        LOG_PRINT(LOGFL_ERRORS,
            ("FltEncrypt!SwapPostDirCtrlBuffersWhenSafe: %wZ Could not lock user buffer, oldB=%p, status=%x\n",
                &p2pCtx->VolCtx->Name,
                iopb->Parameters.DirectoryControl.QueryDirectory.DirectoryBuffer,
                status));
        ...
        if (origBuf == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPostDirCtrlBuffersWhenSafe: %wZ Failed to get System address for MDL: %p\n",
                    &p2pCtx->VolCtx->Name,
                    iopb->Parameters.DirectoryControl.QueryDirectory.MdlAddress));
    ...
    LOG_PRINT(LOGFL_DIRCTRL,
        ("FltEncrypt!SwapPostDirCtrlBuffersWhenSafe: %wZ newB=%p info=%Iu Freeing\n",
            &p2pCtx->VolCtx->Name,
            p2pCtx->SwappedBuffer,
            Data->IoStatus.Information));
...
--*/
{
    ...
    try {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreWriteBuffers:            Error getting volume context, status=%x\n",
                    status));
        ...
        if (newBuf == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreWriteBuffers:            %wZ Failed to allocate %d bytes of memory.\n",
                    &volCtx->Name,
                    writeLen));
        ...
        if (FlagOn(Data->Flags, FLTFL_CALLBACK_DATA_IRP_OPERATION)) {
            ...
            if (newMdl == NULL) {
                ...
                LOG_PRINT(LOGFL_ERRORS,
                    ("FltEncrypt!SwapPreWriteBuffers:            %wZ Failed to allocate MDL.\n",
                        &volCtx->Name));
        ...
        if (iopb->Parameters.Write.MdlAddress != NULL) {
            ...
            if (origBuf == NULL) {
                LOG_PRINT(LOGFL_ERRORS,
                    ("FltEncrypt!SwapPreWriteBuffers:            %wZ Failed to get system address for MDL: %p\n",
                        &volCtx->Name,
        ...
        try {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreWriteBuffers:            %wZ Invalid user buffer, oldB=%p, status=%x\n",
                    &volCtx->Name,
                    origBuf,
                    Data->IoStatus.Status));
        ...
        if (p2pCtx == NULL) {
            ...
            LOG_PRINT(LOGFL_ERRORS,
                ("FltEncrypt!SwapPreWriteBuffers:            %wZ Failed to allocate pre2Post context structure\n",
                    &volCtx->Name));
        ...
        LOG_PRINT(LOGFL_WRITE,
            ("FltEncrypt!SwapPreWriteBuffers:            %wZ newB=%p newMdl=%p oldB=%p oldMdl=%p len=%d\n",
                &volCtx->Name,
                newBuf,
                newMdl,
        ...
        *CompletionContext = p2pCtx;
        ...
        if (NT_SUCCESS(status))
        {
                ...
                FALSE))
            {
                ...
                {
                    DbgPrint("[FltEncrypt] --------- write ---------");

                    DbgPrint("[FltEncrypt] chiper");
                    DbgPrint("[FltEncrypt] size: %x\n", strlen((char*)iopb->Parameters.Write.WriteBuffer));
                    DbgPrint("[FltEncrypt] Write Buffer: %s\n", (char*)iopb->Parameters.Write.WriteBuffer); 
                    ...
                    if (iopb->Parameters.Write.WriteBuffer) {
                        ...
                        char* writeBuffer = (char*)iopb->Parameters.Write.WriteBuffer;
                        int bufferSize = calcRoundBufferSize(strlen((char*)writeBuffer));
                        char* hexarray = ExAllocatePoolWithTag(PagedPool, sizeof(char) * bufferSize, ENCRYPT_TAG);


                        DbgPrint("[FltEncrypt] bufferSize: %d\n", bufferSize);

                        strncpy(hexarray, writeBuffer, bufferSize);
                        DbgPrint("[FltEncrypt] AES Decrypt: %s\n", (char*)hexarray);


                        AES_init_ctx_iv(&ctx, key, iv);
                        AES_CBC_encrypt_buffer(&ctx, (uint8_t*)hexarray, bufferSize);

                        strncpy((char*)Data->Iopb->Parameters.Write.WriteBuffer, hexarray, bufferSize);
                        DbgPrint("[FltEncrypt] AES Encrypt: %s\n", (char*)iopb->Parameters.Write.WriteBuffer);

                        ExFreePoolWithTag(hexarray, ENCRYPT_TAG);
                    }
                }
            }
        }
...
--*/
{
    ...
    LOG_PRINT(LOGFL_WRITE,
        ("FltEncrypt!SwapPostWriteBuffers:           %wZ newB=%p info=%Iu Freeing\n",
            &p2pCtx->VolCtx->Name,
            p2pCtx->SwappedBuffer,
            Data->IoStatus.Information));
